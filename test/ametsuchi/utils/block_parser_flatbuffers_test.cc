...
TEST(BLOCK_PARSER_FLATBUFFERS_TEST, PARSE){
  ...
  std::vector<uint8_t > blob{builder.GetBufferPointer(), builder.GetBufferPointer()+builder.GetSize()};
  ametsuchi::utils::BlockParserFlatbuffers parser(blob);

  ASSERT_EQ(parser.get_length(), 250);
  ASSERT_EQ(parser.get_height(), 500);
  std::array<uint8_t, 32> hash_to_compare;
  std::copy_n(std::make_move_iterator(prev_hash_vector.begin()), 32, hash_to_compare.begin());
  ASSERT_EQ(parser.get_prev_hash(), hash_to_compare);

  std::array<uint8_t, 32> root_to_compare;
  std::copy_n(std::make_move_iterator(merkle_root_vector.begin()), 32, root_to_compare.begin());
  ASSERT_EQ(parser.get_merkle_root(), root_to_compare);

  std::array<uint8_t, 25> created_to_compare;
  std::copy_n(std::make_move_iterator(created_vector.begin()), 25, created_to_compare.begin());
