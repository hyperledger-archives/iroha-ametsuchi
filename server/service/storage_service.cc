...
StorageServiceImpl::StorageServiceImpl() {
  ...
  std::function<bool(iroha::Action)> handler =
  ...
  handler = [this](const iroha::Action &action) {
    ...
    auto signatories = register_signatories.signature();
    if (signatories.empty() || signatories.size() > UINT8_MAX) {
      return false;
    }
    auto account_id = register_signatories.account_id();
    for (const auto &i : signatories) {
      wsv_->add_signatory(account_id, i.public_key());
    }
    return true;
  ...
  handler = [this](const iroha::Action &action) {
    ...
    wsv_->add_peer(add_peer.account_id(), add_peer.address(), 0);
    return true;
  };
...
grpc::Status StorageServiceImpl::GetPeers(
...
grpc::Status StorageServiceImpl::AddBlock(::grpc::ServerContext *context,
  ...
  block.body().SerializeToArray(buffer.data() + block.meta().ByteSize(),
                                block.body().ByteSize());
  // TODO remove cast
  utils::sha3_256((unsigned char *)&hash.at(0), buffer.data(),
                  block.meta().ByteSize() + block.body().ByteSize());
  ...
  for (int i = 0; i < block.body().transaction().size(); ++i) {
    ...
    // Transaction hash
    transaction.meta().SerializeToArray(buffer.data(),
                                        transaction.meta().ByteSize());
    transaction.body().SerializeToArray(
        buffer.data() + transaction.meta().ByteSize(),
        transaction.body().ByteSize());
    utils::sha3_256(
        (unsigned char *)&hash.at(0), buffer.data(),
        transaction.meta().ByteSize() + transaction.body().ByteSize());
    transaction_index_->add_txhash_blockhash_txid(hash, block.meta().height(),
                                                  i);
    // WSV
    for (const auto &action : transaction.body().action()) {
      auto command = action.command_case();
      auto handler = handler_.at(command);
      auto result = handler(action);
      if (!result) {
        return grpc::Status(grpc::StatusCode::INVALID_ARGUMENT,
                            "Failed to apply action to WSV");
      }
    }
  }
...
grpc::Status StorageServiceImpl::GetBlock(
    ::grpc::ServerContext *context, const ::iroha::BlockIdMessage *request,
    ::iroha::BlockMessage *response) {
  uint32_t height;
  if (request->id_case() == iroha::BlockIdMessage::kHeight) {
    ...
    auto block_hash = transaction_index_->get_blockhash_by_txhash(
  ...
  if (height > block_store_->last_id()) {
    return grpc::Status(grpc::StatusCode::OUT_OF_RANGE,
                        "Block id greater than last id");
  }

  auto blob = block_store_->get(height);
  if (blob.empty()) {
    return grpc::Status(grpc::StatusCode::NOT_FOUND, "Block not found");
  }

  auto status = response->mutable_block()->ParseFromArray(blob.data(), blob.size());
  if (!status) {
...
grpc::Status StorageServiceImpl::EraseBlock(
    ::grpc::ServerContext *context, const ::iroha::BlockIdMessage *request,
    ::google::protobuf::Empty *response) {
...
grpc::Status StorageServiceImpl::GetBlocks(
  ...
  if (request->begin() > block_store_->last_id() ||
      request->end() > block_store_->last_id()) {
    return grpc::Status(grpc::StatusCode::OUT_OF_RANGE,
                        "Block id greater than last id");
  }
  auto begin = block_store_->begin() + request->begin(),
       end = block_store_->begin() + request->end();
  for (; begin != end; ++begin) {
    auto blob = *begin;
    if (blob.empty()) {
      return grpc::Status(grpc::StatusCode::NOT_FOUND, "Block not found");
